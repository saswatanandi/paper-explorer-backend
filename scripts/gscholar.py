...
class GoogleScholarScraper:
    ...
    async def search_paper(self, paper_title):
        ...
        try:
            ...
            if title_elem:
                ...
                if paper_info["url"]:
                    print(f"Found URL: {paper_info['url']}")
    ...
    async def close(self):
        """Close the browser with proper cleanup"""
        if self.browser and self.initialized:
            try:
                print("Closing browser...")

                # Try all possible browser closing methods
                if hasattr(self.browser, 'close'):
                    try:
                        ...
                        print(f"Error with browser.close(): {e}")

                elif hasattr(self.browser, 'quit'):
                    try:
                        result = self.browser.quit()
                        if hasattr(result, '__await__'):
                            await result
                    except Exception as e:
                        print(f"Error with browser.quit(): {e}")

                # Allow time for resources to be released
                await asyncio.sleep(1)
            finally:
                # Always mark as closed regardless of success
                self.initialized = False
                self.browser = None
